...
[절대 준수 규칙]
- 원문의 날짜, 숫자, 금액, 연도, 고유명사는 절대 임의로 수정하지 마세요.
- 문서에 '26년이라고 적혀 있으면 그대로 '26년입니다. 현재 연도와 다르다고 오기로 판단하지 마세요.
- 날짜·숫자의 "오류"는 문서 내부에서 앞뒤가 모순될 때만 지적하세요 (예: 본문에는 3월, 표에는 4월).
- 외부 지식이나 추측으로 원문 내용이 틀렸다고 판단하지 마세요.

검사 항목:
1. **논리 구조(logic)**: 서술 흐름 단절, 근거 없는 주장, 결론과 본문 불일치, 항목 순서 문제
2. **숫자/금액(number)**: 아라비아/한글 숫자 혼용, 단위 불일치, 표 합계 오류, 금액 계산 오류, 퍼센트 불일치
3. **날짜/기간(date)**: 문서 내부의 날짜 형식 혼용(2025.03.15 vs 2025년 3월 15일), 문서 내부의 기간 계산 오류, 문서 내부의 앞뒤 날짜 불일치. 단, 연도 자체가 틀렸다고 추측하지 마세요.
...
심각도 기준:
- error: 내용이 틀렸거나 문서의 신뢰성에 영향 (금액 오류, 문서 내부 날짜 모순, 논리 모순)
...
export async function inspectDocument(
  ...
  try {
    rawText = await callGemini({
      prompt,
      signal,
      systemInstruction: '당신은 한국어 공문서 교열 및 논리 검토 전문가입니다. 원문의 날짜·숫자·고유명사는 절대 임의로 변경하지 마세요. 문서 내부의 모순만 지적하세요. 지시한 JSON 스키마만 정확히 출력하세요.',
    });
  } catch (err) {
    if (err instanceof DOMException && err.name === 'AbortError') throw err;
