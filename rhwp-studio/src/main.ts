...
async function initializeDocument(docInfo: DocumentInfo, displayName: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (report.count > 0) {
        // 임베드(iframe)에서는 모달 없이 자동 보정 — 호스트 앱이 흐름을 제어
        const embedded = window.parent !== window;
        const choice = embedded ? 'auto-fix' : await showValidationModalIfNeeded(report);
...
function notifyHwpxSaveModeIfNeeded(): void {
  if (window.parent !== window) return; // 임베드(iframe) 시 저장 안내 토스트 생략
