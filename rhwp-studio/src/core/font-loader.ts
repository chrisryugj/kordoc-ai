...
const FONT_LIST: FontEntry[] = [
  ...
  // === 한컴 SPUA-A 글머리표·기호 (U+F0000~, OFL/한컴 자유배포 함초롬체 subset) ===
  // CDN 함초롬바탕 woff(눈누 subset)에는 SPUA-A 글리프가 빠져 있어 글머리표(□=U+F03DA 등)가
  // .notdef tofu 로 렌더됐다. OS HANBatang(HCR Batang)의 SPUA-A 영역만 subset 한 번들 폰트로
  // '함초롬바탕' family 를 보강한다. unicode-range 로 SPUA-A 만 매칭 → 일반 한글/기존 CDN
  // 글리프에 영향 0. CDN @font-face 보다 뒤에 선언되어 SPUA-A 코드포인트는 이 규칙이 우선한다.
  // generic_fallback 체인(renderer/mod.rs) sans/serif 양쪽 모두 말단에 '함초롬바탕' 을 포함하므로
  // 글머리표 원본 폰트가 무엇이든 폴백으로 이 글리프에 도달한다.
  { name: '함초롬바탕', file: 'fonts/HCRBatang-SPUA.woff2', unicodeRange: 'U+F0000-FFFFD' },
  // 글머리표가 직접 지정하는 1순위 폰트도 SPUA-A 영역을 이 글리프로 보강(폴백 의존 제거).
  // 실측: 글머리표 □(U+F03DA) run 의 원본 폰트는 'HY견고딕'(→NotoSansKR-Bold, SPUA 없음)이었다.
  { name: 'HY견고딕', file: 'fonts/HCRBatang-SPUA.woff2', unicodeRange: 'U+F0000-FFFFD' },
  { name: 'HY중고딕', file: 'fonts/HCRBatang-SPUA.woff2', unicodeRange: 'U+F0000-FFFFD' },
  { name: 'HY신명조', file: 'fonts/HCRBatang-SPUA.woff2', unicodeRange: 'U+F0000-FFFFD' },
  { name: 'HY견명조', file: 'fonts/HCRBatang-SPUA.woff2', unicodeRange: 'U+F0000-FFFFD' },
  { name: '함초롬돋움', file: 'fonts/HCRBatang-SPUA.woff2', unicodeRange: 'U+F0000-FFFFD' },
];
    ...
    await Promise.all(batch.map(async (f) => {
      try {
        ...
        for (const name of names) {
          // unicode-range 지정 폰트(예: SPUA-A 글머리표 subset)는 FontFace 에도 동일 descriptor 를
          // 전달해야 한다. 누락 시 전체 범위로 add 되어 같은 family 의 일반 폰트와 충돌, 글리프 폴백이
          // 막혀 tofu 가 된다(CSS @font-face 의 unicode-range 가 무력화됨).
          const face = new FontFace(name, `url(${f.file}) format('${fmt}')`,
            f.unicodeRange ? { unicodeRange: f.unicodeRange } : undefined);
