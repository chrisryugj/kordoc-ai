...
export default defineConfig({
  // kordoc-ai가 same-origin으로 serve할 경로(/rhwp-studio/). 양방향 postMessage(저장 등)를 위해
  // cross-origin dev(7700) 대신 호스트 앱과 같은 origin에서 구동한다.
  base: '/rhwp-studio/',
  build: {
    outDir: resolve(__dirname, '..', 'public', 'rhwp-studio'),
    emptyOutDir: true,
  },
  define: {
  ...
  resolve: {
    alias: {
      ...
      // 레포 빌드 산출물(../pkg) 대신 kordoc-ai가 쓰는 동일 @rhwp/core(npm, 0.7.15)로 연결
  ...
  server: {
    ...
    fs: {
      ...
      allow: [__dirname, resolve(__dirname, 'node_modules', '@rhwp', 'core'), resolve(__dirname, '..', 'samples')],
    },
  },
  plugins: [
